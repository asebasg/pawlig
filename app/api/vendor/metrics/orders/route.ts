/**
 * GET /api/vendor/metrics/orders
 * Descripción: Obtiene la distribución de órdenes por estado.
...
import { getVendorOrders } from "@/lib/services/vendor-metrics.service";
...
export async function GET(req: NextRequest) {
  try {
    ...
    const ordersData = await getVendorOrders(vendorId, validation.data as VendorMetricsFilters);

    return NextResponse.json(ordersData);
  } catch (error) {
    console.error("[GET_VENDOR_ORDERS_STATUS]", error);
    return NextResponse.json(
