/**
 * GET /api/vendor/metrics/trends
 * Descripción: Obtiene datos agrupados por día para gráficos de tendencia (ventas y cantidad).
...
export async function GET(req: NextRequest) {
  try {
    ...
    const data = await getVendorTrends(vendorId, validation.data as VendorMetricsFilters);

    return NextResponse.json({ data });
  } catch (error) {
    console.error("[GET_VENDOR_TRENDS]", error);
    return NextResponse.json(
