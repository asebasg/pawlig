/**
 * GET /api/vendor/metrics/export
 * Descripción: Exporta reportes de ventas del vendedor en formato CSV, Excel o PDF.
 * Requiere: Sesión activa con rol VENDOR.
 * Implementa: HU-012 (Exportación).
 */
...
export async function GET(req: NextRequest) {
  try {
    ...
    if (!session || session.user.role !== UserRole.VENDOR || !session.user.vendorId) {
    ...
    const products = await getVendorTopProducts(vendorId, filters as VendorMetricsFilters);

    const timestamp = new Date().toISOString().split("T")[0];
    const filename = `reporte-ventas-vendedor-${timestamp}`;
    ...
    const exportOptions: ExportOptions<TopProductData> = {
      ...
      headers: [
        ...
        { key: "revenue", label: "Ingresos (COP)" },
      ],
      filename,
      title: "Reporte de Productos Más Vendidos",
      subtitle: "Panel de Vendedor - PawLig",
    };
    ...
    console.error("[EXPORT_VENDOR_SALES]", error);
    return NextResponse.json(
