...
import { sendNewAdoptionRequestEmail } from "@/lib/services/email.service";
...
export async function GET(req: NextRequest) {
  try {
    ...
    if (
      statusParam &&
      Object.values(AdoptionStatus).includes(statusParam as AdoptionStatus)
    ) {
...
export async function POST(req: NextRequest) {
  try {
    ...
    // Enviar notificación al albergue
    if (adoption.pet.shelter.user?.email) {
      sendNewAdoptionRequestEmail({
        to: adoption.pet.shelter.user.email,
        shelterName: adoption.pet.shelter.name,
        adopterName: session.user.name || "Usuario",
        petName: adoption.pet.name,
        adoptionId: adoption.id,
      }).catch((err) =>
        console.error("Error enviando email nueva adopción:", err),
      );
    }
