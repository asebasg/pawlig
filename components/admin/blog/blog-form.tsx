...
const ACCEPTED_IMAGE_TYPES_BLOG = [
  "image/jpeg",
  "image/png",
  "image/webp",
] as const;
    ...
    <>
      ...
      >
        ...
        <input
          ...
          placeholder={
            tags.length === 0 ? "Ej: nutrición, salud, perros..." : ""
          }
...
export function BlogForm({ initialData }: BlogFormProps) {
  ...
  const {
    ...
    setImageItems: (items) => {
      setGalleryItems(
        items.filter((i) => galleryItems.some((g) => g.id === i.id)),
      );
      setFeaturedItems(
        items.filter((i) => featuredItems.some((f) => f.id === i.id)),
      );
    },
  });
  ...
  const handleEditorImageUpload = useCallback(
    async (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      if (
        !(ACCEPTED_IMAGE_TYPES_BLOG as readonly string[]).includes(file.type)
      ) {
      ...
      try {
        ...
        const response = await fetch("/api/upload", {
          ...
          body: JSON.stringify({
            image: base64,
            folder: CLOUDINARY_FOLDERS.BLOG,
          }),
        });
  ...
  const removeGalleryImage = useCallback(
    async (id: string) => {
      ...
      // Si la imagen está en el HTML del contenido, remover la etiqueta img correspondiente
      if (item.cloudinaryUrl) {
        const currentContent = form.getValues("content");
        if (currentContent && currentContent.includes(item.cloudinaryUrl)) {
          const escaped = item.cloudinaryUrl.replace(
            /[.*+?^${}()|[\]\\]/g,
            "\\$&",
          );
      ...
      try {
        ...
        toast.error(
          "La imagen se quitó de la galería, pero hubo un error al borrarla del servidor.",
        );
      }
    },
    [galleryItems, form],
  ...
  // ---------------------------------------------------------------------------
  // handleEditorImageDelete — callback cuando se borra una imagen desde el editor
  // ---------------------------------------------------------------------------

  const handleEditorImageDelete = useCallback(
    async (src: string) => {
      const item = galleryItems.find((i) => i.cloudinaryUrl === src);
      if (item) {
        await removeGalleryImage(item.id);
      } else {
        // Si proviene de un artículo existente pre-cargado
        try {
          ...
          if (publicId) {
            await fetch("/api/cloudinary/delete", {
              method: "DELETE",
              headers: { "Content-Type": "application/json" },
              body: JSON.stringify({ publicId, url: src }),
            });
          }
          toast.success("Imagen eliminada correctamente.");
        } catch (err) {
    ...
    [galleryItems, removeGalleryImage],
  );
    ...
    async (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error ? err.message : "Error desconocido";
        setFeaturedItems([
          { ...newItem, status: "error", error: errorMessage },
        ]);
        toast.error(`No se pudo subir la imagen: ${errorMessage}`);
      } finally {
        e.target.value = "";
  ...
  return (
    <>
      <motion.form
        ...
        initial={
          prefersReduced ? { opacity: 0 } : { opacity: 0, scale: 0.95, y: 16 }
        }
        animate={
          prefersReduced ? { opacity: 1 } : { opacity: 1, scale: 1, y: 0 }
        }
        exit={
          prefersReduced ? { opacity: 0 } : { opacity: 0, scale: 0.95, y: 8 }
        }
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="col-span-1 md:col-span-2">
            ...
            <textarea
              ...
              aria-describedby={
                form.formState.errors.excerpt ? "excerpt-error" : undefined
              }
              className={cn(
          ...
          <div className="col-span-1 md:col-span-2">
            ...
            <p className="text-xs text-zinc-500 mb-3">
              Opcional. Sube 1 foto para encabezar el artículo. Formatos: JPEG,
              PNG, WEBP.
            </p>
