...
## 8. Reglas para Commits

Para garantizar un historial de cambios profesional, coherente y fácil de auditar, todos los commits deben seguir una estructura estandarizada basada en los patrones históricos del proyecto.

- **Formato**: `tipo(ámbito): Descripción`
- **Idioma**: Español.
- **Tipos Permitidos**:
  - `feat`: Nuevas funcionalidades (ej: `feat(auth): ...`).
  - `fix`: Corrección de errores o bugs.
  - `docs`: Cambios en la documentación o archivos markdown.
  - `style`: Cambios de formato o diseño que no afectan la lógica (CSS, espacios).
  ...
  - `improvement`: Mejoras en funcionalidades ya existentes.
- **Ámbito (Scope)**: Opcional pero recomendado cuando el cambio es específico. Se escribe en minúsculas. Si afecta a múltiples áreas, se separan por comas (ej: `feat(ui, pets): ...`).
- **Gramática de la Descripción**:
  - Se permite el inicio tanto en Mayúscula como en minúscula (ej: `Agrega` o `actualizar`).
  - Estructura: Se deben usar verbos en presente (3ª persona singular), infinitivos o sustantivos descriptivos.
  - No usar gerundios (ej: `Agregando`).
- **Plantilla**:
  ```bash
  <type>[optional scope]: Acción realizada en español (Mayúscula o minúscula inicial).
...
- `docs: Agrega el archivo CONTEXT.md.`
- `feat(shelter/pets): Desarrolla la página para visualizar y gestionar las mascotas del albergue.`
- `style(changelog): actualizar diseño y añadir nuevas características`
- `fix(adoptions): Corrección del filtro de sexo en la galería.`
